...
class IntervalsSync:
    ...
    def _intervals_get_date_chunks(self, endpoint: str, oldest: str, newest: str,
                                   chunk_days: int = 180) -> List[Dict]:
        """Fetch a long date range in bounded chunks and de-duplicate records.

        Some API/listing paths can return incomplete results for very large windows.
        Splitting the request into smaller inclusive date ranges avoids silent truncation
        without relying on undocumented pagination parameters. Records are de-duplicated
        by ``id`` when present, otherwise by a stable JSON fingerprint.
        """
        ...
        while cursor <= end:
            ...
            params = {
                "oldest": cursor.strftime("%Y-%m-%d"),
                "newest": chunk_end.strftime("%Y-%m-%d"),
            }
            payload = self._intervals_get(endpoint, params)
            if not isinstance(payload, list):
                raise ValueError(f"Expected list payload from {endpoint}, got {type(payload).__name__}")
            ...
            for item in payload:
                ...
                if item_id is not None:
                    key = (endpoint, str(item_id))
                else:
                    key = (endpoint, json.dumps(item, sort_keys=True, default=str))
                if key in seen:
                    continue
                seen.add(key)
                merged.append(item)
            ...
            if self.debug:
                print(f"    {endpoint}: {params['oldest']} → {params['newest']} ({len(payload)} rows)")
            cursor = chunk_end + timedelta(days=1)

        def _sort_key(item: Dict) -> str:
            return (item.get("start_date_local") or item.get("id") or "")[:19]
    ...
    def should_generate_history(self) -> bool:
        ...
        # Optional one-run override for troubleshooting/backfills.
        # Set FORCE_HISTORY=1 in the workflow environment, then remove it after success.
        if os.getenv("FORCE_HISTORY", "").strip().lower() in {"1", "true", "yes"}:
            if self.debug:
                print("  FORCE_HISTORY enabled — will regenerate history.json")
            return True
    ...
    def generate_history(self) -> Dict:
        ...
        # Fetch the complete available history. Intervals.icu will simply return no activities
        # before the athlete's first recorded activity. A fixed early date avoids
        # silently dropping older peak seasons as time moves forward.
        history_start = os.getenv("HISTORY_START_DATE", "2010-01-01")
        newest = now.strftime("%Y-%m-%d")
        
        # Fetch all activities for full range
        print(f"  Fetching full activity history from {history_start}...")
        try:
            all_activities = self._intervals_get_date_chunks(
                "activities", history_start, newest, chunk_days=180
            )
        except Exception as e:
        ...
        try:
            all_wellness = self._intervals_get_date_chunks(
                "wellness", history_start, newest, chunk_days=366
            )
        except Exception as e:
        ...
        # === FULL ARCHIVE OUTPUTS (Hwang Edition) ===
        # These files are additive and do not alter the Section 11 core schemas.
        # Missing upstream metrics remain null/omitted rather than estimated.
        archive_outputs = self._build_archive_outputs(
    ...
    def _build_archive_outputs(self, all_activities: List[Dict], all_wellness: List[Dict],
                               athlete: Dict, earliest_date: str, latest_date: str,
                               athlete_units: Optional[Dict[str, str]] = None) -> Dict[str, Dict]:
        """Build additive lifetime archive files without changing Section 11 core schemas."""
        generated_at = datetime.now().isoformat()
        yearly = defaultdict(lambda: {
            "activity_count": 0, "total_tss": 0.0, "total_seconds": 0,
            "distance_km": 0.0, "elevation_m": 0.0, "max_activity_tss": None,
            "longest_activity_seconds": 0, "ctl_peak": None, "atl_peak": None,
            "ftp_peak": None, "eftp_peak": None, "weight_values": []
        })

        # Common field variants used by Intervals.icu exports.
        ...
        for a in all_activities:
            ...
            year = date[:4]
            y = yearly[year]
            tss = first_num(a, ["icu_training_load", "training_load", "tss"]) or 0.0
            secs = int(first_num(a, ["moving_time", "elapsed_time"]) or 0)
            distance_m = first_num(a, ["distance"]) or 0.0
            elev = first_num(a, ["total_elevation_gain", "icu_elevation_gain"]) or 0.0
            y["activity_count"] += 1
            y["total_tss"] += tss
            y["total_seconds"] += secs
            y["distance_km"] += distance_m / 1000.0
            y["elevation_m"] += elev
            y["max_activity_tss"] = max(y["max_activity_tss"] or 0, tss)
            y["longest_activity_seconds"] = max(y["longest_activity_seconds"], secs)
            ftp = first_num(a, ["icu_ftp", "ftp"])
            eftp = first_num(a, ["icu_pm_ftp", "icu_pm_cp", "eftp"])
            if ftp:
                y["ftp_peak"] = max(y["ftp_peak"] or 0, ftp)
            if eftp:
                y["eftp_peak"] = max(y["eftp_peak"] or 0, eftp)
            # Full individual activity archive (v3.123-HH).
            # These values come directly from the Intervals.icu activity-list API.
            # Missing values remain absent rather than being estimated.
            elapsed_secs = int(first_num(a, ["elapsed_time"]) or 0)
            avg_speed_mps = first_num(a, ["average_speed", "avg_speed"])
            max_speed_mps = first_num(a, ["max_speed"])
            distance_km = distance_m / 1000.0

            # Prefer the API-provided average speed. When absent, derive only from
            # measured distance and measured moving time.
            avg_speed_kph = (
                avg_speed_mps * 3.6
                if avg_speed_mps is not None
                else (distance_km / (secs / 3600.0) if secs > 0 else None)
            )
            max_speed_kph = max_speed_mps * 3.6 if max_speed_mps is not None else None

            avg_power = first_num(a, [
                "average_watts", "avg_watts", "average_power",
                "avgWatts", "icu_average_watts"
            ])
            normalized_power = first_num(a, [
                "weighted_average_watts", "np", "icu_pm_np",
                "normalizedPower", "icu_weighted_avg_watts"
            ])
            avg_hr = first_num(a, [
                "average_heartrate", "avg_hr", "average_hr"
            ])
            max_hr = first_num(a, [
                "max_heartrate", "max_hr"
            ])
            avg_cadence = first_num(a, [
                "average_cadence", "avg_cadence"
            ])
            intensity = first_num(a, ["icu_intensity", "intensity"])
            variability_index = first_num(a, ["icu_variability_index", "variability_index"])
            decoupling = first_num(a, ["icu_hr_decoupling", "decoupling"])
            efficiency_factor = first_num(a, ["icu_efficiency_factor", "efficiency_factor"])
            calories = first_num(a, ["calories", "kcal"])
            work_kj = first_num(a, ["kilojoules", "work_kj"])
            rpe = first_num(a, ["icu_rpe", "rpe"])
            feel = first_num(a, ["feel"])

            record = {
                "date": date,
                "start_datetime": a.get("start_date_local") or a.get("start_date"),
                "id": a.get("id"),
                "name": a.get("name"),
                "description": a.get("description"),
                "type": a.get("type"),
                "indoor": self._is_indoor_cycling(a.get("type", "")),
                "moving_seconds": secs,
                "elapsed_seconds": elapsed_secs,
                "distance_km": round(distance_km, 2),
                "avg_speed_kph": round(avg_speed_kph, 2) if avg_speed_kph is not None else None,
                "max_speed_kph": round(max_speed_kph, 2) if max_speed_kph is not None else None,
                ...
                "intensity_pct": round(intensity, 1) if intensity is not None else None,
                "variability_index": round(variability_index, 3) if variability_index is not None else None,
                "decoupling_pct": round(decoupling, 2) if decoupling is not None else None,
                "efficiency_factor": round(efficiency_factor, 3) if efficiency_factor is not None else None,
                "tss": round(tss, 1),
                "calories": round(calories) if calories is not None else None,
                "work_kj": round(work_kj, 1) if work_kj is not None else None,
                "rpe": round(rpe, 1) if rpe is not None else None,
                "feel": round(feel, 1) if feel is not None else None,
                "power_zone_times": a.get("icu_zone_times"),
                "hr_zone_times": a.get("icu_hr_zone_times"),
                "interval_summary": a.get("interval_summary"),
            }
            # Keep the archive compact and make absence unambiguous.
            activity_records.append({k: v for k, v in record.items() if v is not None})
        ...
        for w in all_wellness:
            ...
            if len(date) < 4:
                continue
            year = date[:4]
            weight = first_num(w, ["weight"])
            ctl = first_num(w, ["ctl"])
            atl = first_num(w, ["atl"])
            if weight and weight > 0:
                yearly[year]["weight_values"].append(weight)
                weight_history.append({"date": date, "weight_kg": round(weight, 2)})
            if ctl is not None:
                yearly[year]["ctl_peak"] = max(yearly[year]["ctl_peak"] if yearly[year]["ctl_peak"] is not None else ctl, ctl)
            if atl is not None:
                yearly[year]["atl_peak"] = max(yearly[year]["atl_peak"] if yearly[year]["atl_peak"] is not None else atl, atl)

        # All-time and per-year power curves. Failure is non-critical and explicitly recorded.
        anchors = {"5s": 5, "30s": 30, "1m": 60, "5m": 300, "20m": 1200, "60m": 3600}
        curve_windows = [("all_time", earliest_date, latest_date)]
        curve_windows += [(year, f"{year}-01-01", f"{year}-12-31") for year in sorted(yearly)]
        curves_arg = ",".join(f"r.{start}.{end}" for _, start, end in curve_windows)
        curve_payload = None
        curve_error = None
        try:
            curve_payload = self._intervals_get("power-curves", {"type": "Ride", "curves": curves_arg})
        except Exception as e:
            curve_error = str(e)

        curves_by_id = {}
        if isinstance(curve_payload, dict):
            curves_by_id = {c.get("id"): c for c in curve_payload.get("list", []) if isinstance(c, dict) and c.get("id")}

        power_curves = {}
        for label, start, end in curve_windows:
            cid = f"r.{start}.{end}"
            c = curves_by_id.get(cid)
            anchor_values = {}
            if c:
                secs_list, watts_list = c.get("secs", []), c.get("watts", [])
                for name, seconds in anchors.items():
                    value = None
                    if seconds in secs_list:
                        idx = secs_list.index(seconds)
                        if idx < len(watts_list) and isinstance(watts_list[idx], (int, float)) and watts_list[idx] > 0:
                            value = round(float(watts_list[idx]), 1)
                    anchor_values[name] = value
            else:
                anchor_values = {name: None for name in anchors}
            power_curves[label] = {"start": start, "end": end, "anchors_watts": anchor_values}

        personal_bests = {
            "generated_at": generated_at,
            "source": "Intervals.icu API",
            "data_range": {"earliest": earliest_date, "latest": latest_date},
            "power": power_curves.get("all_time", {}).get("anchors_watts", {}),
            "power_note": "Measured MMP anchors from Intervals.icu power-curves; null means unavailable.",
            "ctl_peak": max((v["ctl_peak"] for v in yearly.values() if v["ctl_peak"] is not None), default=None),
            "atl_peak": max((v["atl_peak"] for v in yearly.values() if v["atl_peak"] is not None), default=None),
            "ftp_peak_reported": max((v["ftp_peak"] for v in yearly.values() if v["ftp_peak"] is not None), default=None),
            "eftp_peak_reported": max((v["eftp_peak"] for v in yearly.values() if v["eftp_peak"] is not None), default=None),
            "activity_tss_peak": max((v["max_activity_tss"] for v in yearly.values() if v["max_activity_tss"] is not None), default=None),
            "longest_activity_seconds": max((v["longest_activity_seconds"] for v in yearly.values()), default=0),
        }

        yearly_rows = []
        for year in sorted(yearly):
            y = yearly[year]
            weights = y.pop("weight_values")
            year_power = power_curves.get(year, {}).get("anchors_watts", {})
            yearly_rows.append({
                "year": int(year), "activity_count": y["activity_count"],
                "total_hours": round(y["total_seconds"] / 3600.0, 1),
                "total_tss": round(y["total_tss"], 0),
                "distance_km": round(y["distance_km"], 1),
                "elevation_m": round(y["elevation_m"], 0),
                "ctl_peak": round(y["ctl_peak"], 1) if y["ctl_peak"] is not None else None,
                "atl_peak": round(y["atl_peak"], 1) if y["atl_peak"] is not None else None,
                "ftp_peak_reported": round(y["ftp_peak"], 1) if y["ftp_peak"] else None,
                "eftp_peak_reported": round(y["eftp_peak"], 1) if y["eftp_peak"] else None,
                "avg_weight_kg": round(statistics.mean(weights), 2) if weights else None,
                "longest_activity_hours": round(y["longest_activity_seconds"] / 3600.0, 2),
                "max_activity_tss": round(y["max_activity_tss"], 1) if y["max_activity_tss"] is not None else None,
                "power_curve_watts": year_power,
            })

        yearly_summary = {"generated_at": generated_at, "source": "Intervals.icu API", "years": yearly_rows}
        power_curve = {
            "generated_at": generated_at, "source": "Intervals.icu power-curves API",
            "curve_error": curve_error, "anchors_seconds": anchors, "curves": power_curves,
            "note": "Values are measured mean-maximal power anchors, not FTP estimates."
        }
        weight_payload = {
            "generated_at": generated_at, "source": "Intervals.icu wellness API",
        ...
        archive = {
            ...
            "activity_schema_note": (
                "Individual activities are exported from the Intervals.icu activity-list API. "
                "avg_speed_kph uses the API average_speed field when available; otherwise it is "
                "derived from measured distance and moving time. Missing metrics are omitted."
            ),
            "notes": [
                "Core Section 11 files remain unchanged.",
                "No missing metric is estimated; unavailable values are omitted.",
                "Duplicate handling remains upstream in Intervals.icu; this export preserves returned activity IDs."
            ]
        }
        return {
            self.ARCHIVE_FILE: archive,
            self.YEARLY_SUMMARY_FILE: yearly_summary,
    ...
    def _build_history_summaries(self, daily_90d: List[Dict], weekly_180d: List[Dict],
        ...
        for key in ["monthly_1y", "monthly_2y", "monthly_3y", "monthly_5y", "monthly_10y"]:
...
def main():
    ...
    if args.generate_history:
        print(f"\n📊 Generating full history and lifetime archive files...")
        ...
        if github_token and github_repo and not args.output:
            ...
            for archive_name in [sync.ARCHIVE_FILE, sync.YEARLY_SUMMARY_FILE, sync.PERSONAL_BESTS_FILE, sync.POWER_CURVE_FILE, sync.WEIGHT_HISTORY_FILE]:
                archive_path = sync.data_dir / archive_name
                if archive_path.exists():
                    with open(archive_path, "r") as f:
                        archive_payload = json.load(f)
                    sync.publish_to_github(archive_payload, filepath=archive_name,
                                           commit_message=f"Generate {archive_name} - {datetime.now().strftime('%Y-%m-%d')}")
                    print(f"   ✅ {archive_name} pushed to GitHub")
        return
                ...
                for archive_name in [sync.ARCHIVE_FILE, sync.YEARLY_SUMMARY_FILE, sync.PERSONAL_BESTS_FILE, sync.POWER_CURVE_FILE, sync.WEIGHT_HISTORY_FILE]:
                    ...
                    if archive_path.exists():
                        with open(archive_path, "r") as f:
                            archive_payload = json.load(f)
                        sync.publish_to_github(archive_payload, filepath=archive_name,
                                               commit_message=f"Auto-generate {archive_name} - {datetime.now().strftime('%Y-%m-%d')}")
                        print(f"   ✅ {archive_name} pushed to GitHub")
        except Exception as e:
