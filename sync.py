...
class IntervalsSync:
    ...
    def generate_history(self) -> Dict:
        ...
        # Determine how far back we can go (up to 5 years)
        earliest_5y = (now - timedelta(days=365 * 5)).strftime("%Y-%m-%d")
        newest = now.strftime("%Y-%m-%d")
        
        # Fetch all activities for full range
        print("  Fetching full activity history (up to 5 years)...")
        try:
            all_activities = self._intervals_get("activities", {
                "oldest": earliest_5y, "newest": newest
            })
        except Exception as e:
